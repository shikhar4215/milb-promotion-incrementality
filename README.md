...
**Status: complete.** SQL warehouse, analysis, five-page Power BI report, and write-up.
...
The star schema the dashboard reads is built by SQL views over a SQLite warehouse, not
by pandas. The model stays in Python — fitting OLS with clustered errors and a 400-draw
placebo belongs in statsmodels — but everything downstream of the model's residuals is
SQL.

The piece worth reading is homestand reconstruction. A homestand is a run of home dates
uninterrupted by a road game, which is a gaps-and-islands problem: `LAG` finds every
date whose previous appearance was a road date, and a running `SUM` over those flags
numbers the runs.

`validate_warehouse.py` compares the SQL output against the pandas implementation column
by column and exits non-zero on any mismatch. It caught three real defects, including a
phantom road date — 32 club-dates in the panel are listed both home and away, and
keeping both rows split 545 dates into the wrong homestands. Full detail, including the
one difference that remains and why it is left in:
[`docs/sql-warehouse.md`](docs/sql-warehouse.md).
...
data/
  ...
  processed/    analysis-ready tables; bi/ is the star schema Power BI reads,
                built by SQL, and bi_reference/ is the pandas copy it is checked against
  warehouse/    milb.db - SQLite, committed so the queries run without re-extracting
sql/
  01_schema.sql    DDL and indexes, with the grain of each table stated
  02_views.sql     the star schema: homestand reconstruction as gaps-and-islands,
                   nearest-giveaway offset, fact and club dimension
  03_analysis.sql  query library; every query reproduces a number in the write-up
src/
  ...
  build_bi_tables.py          star schema in pandas - the reference implementation
  load_warehouse.py           builds SQLite and exports the star schema from SQL
  validate_warehouse.py       checks the SQL output against the pandas one, 60 checks
...
python3 src/build_bi_tables.py        # pandas reference copy
python3 src/load_warehouse.py         # build SQLite, export the star schema from SQL
python3 src/validate_warehouse.py     # assert the two agree
```

The database is committed, so the query library runs without any of the above:

```bash
sqlite3 data/warehouse/milb.db < sql/03_analysis.sql
```
